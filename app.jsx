...
async function saveRegisteredAccount(email, username, password, code, is_admin) {
  try {
...
function OtpLogin({ onLoginSuccess }) {
  ...
  async function handlePasswordSignIn() {
    ...
    try {
      ...
      if (!account) {
        setLoading(false);
        setError("Account not found. Please click 'Create Account' to sign up.");
        return;
      }

      // Verify password (support both SHA-256 hash and auto-upgrading legacy plaintext)
              ...
              <>
                ...
                <div style={{ position: "relative" }}>
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        if (authMode === "signup") handleSendSignupOtp();
